...
## LOCKED: The submission form's backend is a small serverless function, kept separate from the publishing pipeline
...
**A dedicated function does exactly three things** (accept the submission, generate the token, run the check) and writes nothing to `ring.json` directly. A passing submission still becomes a pull request through the existing human spot-check and existing pipeline, unchanged from today.
...
Still open, and recorded rather than assumed: which serverless platform, how the function authenticates to open a PR on a submitter's behalf, and what happens to a submission that fails the reachability check on its first attempt. See `submission-form-spec.md` section 7 and `open-questions.md`.
...
**Left open, and now larger than it looked before this round:** which serverless platform the intake function runs on, how it authenticates to open the final PR, and (new as of this decision) where the review queue itself is stored and what a maintainer actually looks at to approve or reject a pending submission. That last one is a real second surface this design introduces, not an implementation detail of the already-decided intake function, and it should be decided on its own terms rather than assumed to come for free alongside it.
