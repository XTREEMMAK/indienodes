...
**The ring repository.** `members/`, both schemas, and the build/validate/health scripts now also live in `github.com/XTREEMMAK/indienodes-ring`, with their own CI (mirroring this repo's own push-triggered "Ring data" check, which the copied PR-only workflows did not by themselves reproduce) and pre-push hook. This is additive, not a cutover: `indienodes-app` still owns its own copy of everything moved, and remains the thing every client actually reads from, until a canonical publish endpoint exists and this repo is repointed at it. Notably, the n8n submission pipeline still opens pull requests against this repo, not the new one, and re-scoping that is unstarted on purpose: it touches a live credential and the form intake it gates, and is being treated as its own deliberate step rather than folded into the file move.
