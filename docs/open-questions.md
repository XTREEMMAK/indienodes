...
- **`KOFI_URL` is still a literal `TODO`.** `src/lib/config.js` has `https://ko-fi.com/TODO`, the Support tab's donate button in the About modal. Needs the real Ko-fi handle; unlike the domain, it can't be guessed.
