...
- ~~**Tapping a node links out instead of opening the reader.**~~ Resolved by amending the brief rather than the app: brief v0.3 LOCKS that the reader opens from a labelled control on the card, not by tapping the card. The original wording predated the card carrying Visit, a like toggle, and a drag surface. Each type that gains a reader gets its own control; comics have one now.
...
- **`GITHUB_URL` and `KOFI_URL` are still literal `TODO`s.** `src/lib/config.js` has `https://github.com/TODO/indienode-v2` and `https://ko-fi.com/TODO`. Both are live links in the About modal (Source & License, and the Support tab's donate button), and the release list builds every one of its anchors from `GITHUB_URL`, so the whole release history currently links nowhere. Needs the real GitHub org/repo and the real Ko-fi handle; unlike the domain, neither can be guessed.
...
- **Where the EULA and the visitor-facing terms live, and what they cover.** `submission-form-spec.md` section 4 is a _submitter_ consent clause and nothing else. Publishing plausibly also wants Terms of Use for visitors and a privacy notice; the privacy notice is unusually easy here (no accounts, no server-side data, everything local), and the discovery journal makes it more necessary rather than less, even though nothing leaves the browser. None of this can be rendered anywhere yet, since there is no submission form: `/join` documents pull requests and issues, with an explicit interim notice saying so. The sequence is form, then EULA.
...
- **The submission form's own remaining unknowns.** ~~What happens to a submission that fails the reachability check on its first attempt~~ is resolved: the check is synchronous and inside the form (locked), so retry is just pressing Verify again. ~~Which serverless platform runs the intake function~~ is resolved: n8n, over a single webhook, with a deliberately vendor-neutral `VITE_SUBMISSION_WEBHOOK_URL` (see `decisions.md`). Still genuinely open: how the workflow authenticates to open the final pull request, and whether that PR still needs its own separate merge click given a maintainer already approved the submission a step earlier in the review queue. On the second: the working recommendation is **yes, keep the merge click**, because that is where `npm run validate:publish` actually runs against the composed file, and a maintainer approving an entry is not the same act as confirming the file it produced still validates. See `submission-form-spec.md` section 7.
...
- **`GITHUB_URL` and `KOFI_URL` in `src/lib/config.js` are still literal `TODO` placeholders, and the first one is now blocking rather than cosmetic.** It was harmless while `/join` merely linked to a repo that did not exist yet. It stops being harmless the moment the submission workflow opens pull requests against that repo on a submitter's behalf: the real owner/name has to be settled before the bot has somewhere to write.
